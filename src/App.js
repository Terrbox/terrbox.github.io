    ...
    <ThemeProvider theme={darkTheme}>
      ...
      <main>
          ...
          <div style={{margin:20}}>
            ...
            <Paper className="toolbar" style={{marginTop:10}}><Button onClick={() => {nextTurn()}}>Next turn</Button> {playerCount > 0 && <><span>XP: {exp}</span> <span>-</span> <span style={{color:"lightgreen", fontWeight: exp <= trivial || (exp > trivial && exp < low) ? "bold" : "normal", textDecoration: exp <= trivial || (exp > trivial && exp < low) ? "underline" : "none"}}>Trivial {trivial}XP</span> <span style={{color:"green", fontWeight: exp >= low && exp < moderate ? "bold" : "normal", textDecoration: exp >= low && exp < moderate ? "underline" : "none"}}>Low {low}XP</span> <span style={{color:"yellow", fontWeight: exp >= moderate && exp < severe ? "bold" : "normal", textDecoration: exp >= moderate && exp < severe ? "underline" : "none"}}>Moderate {moderate}XP</span> <span style={{color:"orange", fontWeight: exp >= severe && exp < extreme ? "bold" : "normal", textDecoration: exp >= severe && exp < extreme ? "underline" : "none"}}>Severe {severe}XP</span> <span style={{color:"red", fontWeight: exp >= extreme ? "bold" : "normal", textDecoration: exp >= extreme ? "underline" : "none"}}>Extreme {extreme}XP</span></>} <Button onClick={() => {setList([]); setPlayerCount(0);}} style={{float:"right"}}>Clear</Button> <Button onClick={() => {clearEnemies()}} style={{float:"right"}}>Clear enemies</Button> <Button onClick={() => {orderByInitiative()}} style={{float:"right"}}>Order by Initiative</Button> </Paper>
